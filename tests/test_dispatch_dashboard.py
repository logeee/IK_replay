...
class DispatchDashboardTests(unittest.TestCase):
    def test_root_serves_workflow_dashboard(self):
        ...
        self.assertIn('const DEXTEROUS_LTR_LANGUAGE =', html)
        self.assertIn("新模式固定左→右", html)
        self.assertIn('$("manualWorkflow").addEventListener("change"', html)
        ...
        self.assertIn('id="manualCarousel"', html)
        self.assertIn('id="manualCycles"', html)
        self.assertIn('taskBody.task = "carousel"', html)
        self.assertIn('id="defaultPushForceRemoteToClose"', html)
        self.assertIn('id="defaultPushForceCloseToRemote"', html)
        self.assertIn('id="dexterousStartSpeed"', html)
        self.assertIn('id="dexterousApproachSpeed"', html)
