...
矩形中心）并列的另一种「粉点 → 绿点」模型。差别只在参考点与偏移组织：
    ...
    锚点     = 参考点 + anchor_offset（墙面系）——由标定脚本定义（点 1/点 3
               中点，或旋钮 mask 中心均值）；从不被人工点击
    目的点   = 锚点 + point{1,3}_offset（墙面系）；两点各自独立，不假设等高对称
    去 1 还是 3 默认由置信度最高的旋钮类（旋钮右 → 1，旋钮左 → 3）决定；
    7005 调试可手动指定类别。两种方式都不使用旋钮 mask 的几何。
