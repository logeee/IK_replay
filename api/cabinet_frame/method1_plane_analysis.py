"""柜面坐标系构建·方法一：多平面分析（原 ``api/cabinet_wall_frame.py``）。

纯内存实现，不含数据集查找、标定持久化或文件系统 API（最初迁自
``rgbd_collector.analysis``）。流水线见 :func:`build_wall_coordinate_frame`：

1. ``fit_dominant_plane``：RANSAC + SVD 拟合主平面 → Y 轴（入墙），
   临时 X/Z 取相机上方向投影（``axis_estimation="camera-up-projection"``），
   原点 = 相机原点在柜面上的投影；
2. ``segment_dominant_planes`` / ``split_plane_labels_by_connectivity``：
   多平面分割并按像素连通域拆块，保留最远平面 P0；
3. ``describe_p0_boundary_lines``：各平面与 P0 的相邻边界 RANSAC 拟直线；
4. X 轴三级回退：P0 边界线 → 次平面主轴 → 平面交线。

方法 id = ``core.cabinet_frame_methods.METHOD1_PLANE_ANALYSIS``；分发与
输出契约校验在 ``api/cabinet_frame/__init__.py``。本模块只负责算法。
"""
