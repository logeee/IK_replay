...
YOLO（Xuanniu_hhy.pt 旋钮类）识别的是开关的物理指向「旋钮左/右」，不读
...
class SwitchFlow:
    ...
    # ⚠ 该偏移按最早的 Xuanniu.pt「开关在右」的框标定；现用 Xuanniu_hhy.pt
    # 的「旋钮左/右」框是旋钮本体的小框，au=1.23（框右缘外侧）的几何已不
    # 成立，此像素兜底路径启用前必须重新标定（主路径 7005 点云取点不受
    # 影响，偏移由 0.2.0-s 模型按拟合矩形中心重新推导）。
