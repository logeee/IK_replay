...
`retries` 是每个方向的最大尝试次数。首方向由 YOLO 根据旋钮当前位置选择；成功后手臂保持高位并移动到对向起手式终点再复核，失败则回本方向起手式终点重试。复核成功并准备执行反方向时，会先检查抬手状态下的柜面角度，超出验收范围才扭腰纠正；随后等待腰部与 IMU 稳定，再由 7005 取点。达到 `cycles` 后，从最后所在的起手式终点倒序收尾并释放手臂。左右方向分别使用 17001 保存的对应默认偏移配置。
