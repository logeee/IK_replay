...
| `site` | 可选 | string | `factory` 或 `lab`；不传时使用 17001 保存的默认现场 |
| `target_offset_wall_mm` | 可选 | object | 显式指定本次墙面系取点微调；不传时按 language 选择该方向独立保存的偏移预设 |
| `first_round_offset_wall_mm` | 可选 | object | 只在第 1 轮叠加的墙面系额外偏置；不传时按 language 使用 17001 为该方向保存的首轮默认值，第 2 轮起自动归零 |
...
独立选择的目的点偏移预设及首轮额外偏置。基础偏移与首轮额外偏置的单轴
合计限 ±100mm。识别到目标状态时直接返回“无需拨动”；识别到
拨前状态才执行，拨后必须识别到 language 指定的目标状态才算成功。
...
| 3 | ALIGN_FAILED | 对正柜面失败：抬手前对中不收敛，或抬手后双向纠偏仍无法进入保持带（会保留偏差可信上限、单杆限幅、累计转身预算和运控无响应检查） | 重新走 `/check/flip` 站位后再触发 |
